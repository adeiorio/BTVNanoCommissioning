...
class NanoProcessor(processor.ProcessorABC):
    ...
    def process_shift(self, events, shift_name):
        ...
        if "WtoLNu-4Jets_TuneCP5_13p6TeV_madgraphMLM-pythia8" in dataset:
            no2count_mask = (events.LHE.Njets == 0) | (events.LHE.Njets > 3)
        else:
            no2count_mask = np.ones(len(events), dtype="bool")

        ## Muon cuts
        # muon twiki: https://twiki.cern.ch/twiki/bin/view/CMS/SWGuideMuonIdRun2
        # muon twiki: https://twiki.cern.ch/twiki/bin/view/CMS/MuonRun32022
        iso_muon = events.Muon[(events.Muon.pt > 27) & mu_idiso(events, self._campaign)]
        ...
        mujetsel = ak.fill_none(
            (
                ...
                )  # till here same selection as in JP's code
        ...
        # Additional cuts on soft mu as done by JP
        req_addsoftmu = (
            (soft_muon[:, 0].dxy / soft_muon[:, 0].dxyErr)
            > 1.0
            # & (abs(soft_muon[:, 0].dxy) > 0.2)
        )
        ...
        req_Wmass = (
            Wmass.mass > 55
        )  # this condition seems to be on the mass of the W boson and not on the transverse mass

        # Updated event_level with the new condition for transverse W-boson mass
        mtw_threshold = 55  # Adjust this threshold as needed
        req_mtw = (
        ...
        if self.isArray:
            ...
            # Inspect the structure of the pruned_ev object
            structure = ak.type(pruned_ev)
            ...
            for kin in ["pt", "eta", "phi", "mass", "pfRelIso04_all", "dxy", "dz"]:
                for obj in [
                    ...
                    if "PuppiMET" in obj and not (kin == "pt" or kin == "phi"):
                        # print(f"Excluding: {obj}_{kin}")
                        continue
